...
%Phan tich Ergodic
    for i = 1:length(pc_linear)
        ...
        Ec2 = (R*pi/4)^4 + 6*((R*pi/4)^2)*(R*(16-pi^2)/16) + 3*(R*(16-pi^2)/16)^2 ...
               + 2*sqrt(pi)*((R*pi/4)^3 + 3*(R*(16-pi^2)/16)*(R*pi/4))...
...
% Ve do thi cho cac ket qua
    ...
    xlabel('p_c(in dB))');
    ylabel('R^C_S (in bps/Hz)');
    title('Tỷ lệ Ergodic của mạng di động với SNR tại CT(p_C)');
    grid on;
    ax = gca;
    ax.XMinorGrid = 'on';
...
%Mo phong Ergodic
    for i = 1:length(pc_linear)
        ...
        % Tao kenh Rayleigh theo phan phoi Gaussian
            ...
            ergodic_rate_sum = ergodic_rate_sum + (log2(1 + gamma_L_C)-log2(1 + gamma_E_C));% Ergodic tai E
        end
